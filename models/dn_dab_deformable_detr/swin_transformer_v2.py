...
class WindowAttention(nn.Module):
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0.,
        ...
        assert rpe_output_type== 'sigmoid', 'The released version uses sigmoid.'
        self.rpe_output_type = rpe_output_type
        self.relative_coords_table_type = relative_coords_table_type

        assert self.attn_type == 'cosine_mh', 'The released version is cosine_mh.'
        if self.attn_type == 'cosine_mh' or self.attn_type == "cosine_mh_eps1e-4" or self.attn_type == "cosine_mh_addeps":
        ...
        assert self.relative_coords_table_type != 'none', 'The released version uses continuous rpb.'
    ...
    def forward(self, x, mask=None):
        ...
        elif self.attn_type == 'cosine_mh_eps1e-4':
            q = custom_normalize(q.float(), dim=-1, eps=1e-4)
            k = custom_normalize(k.float(), dim=-1, eps=1e-4)
            logit_scale = torch.clamp(self.logit_scale, max=torch.log(torch.tensor(1. / 0.01))).exp()
            attn = (q @ k.transpose(-2, -1)) * logit_scale.float()
        elif self.attn_type == 'cosine_mh_addeps':
            q = F.normalize(q.float() + 1e-5, dim=-1, eps=1e-8)
            k = F.normalize(k.float() + 1e-5, dim=-1, eps=1e-8)
            logit_scale = torch.clamp(self.logit_scale, max=torch.log(torch.tensor(1. / 0.01))).exp()
            attn = (q @ k.transpose(-2, -1)) * logit_scale.float()
        elif self.attn_type == 'normal':
            q = q * self.scale
...
class SwinTransformerBlockPost(nn.Module):
    ...
    def __init__(self, dim, num_heads, window_size=7, shift_size=0,
                 ...
                 relative_coords_table_type='norm8_log', rpe_hidden_dim=512,
                 rpe_output_type='normal', attn_type='normal', mlpfp32=False,
        ...
        assert not self.endnorm, 'The released version does not use endnorm.'
        if self.endnorm:
...
class BasicLayer(nn.Module):
    ...
    def __init__(self,
        ...
        # build blocks
        assert self.postnorm, 'The released version only uses postnorm.'
        if self.postnorm:
    ...
    def __init__(self,
        ...
        for i_layer in range(self.num_layers):
            ...
            if i_layer <= self.num_layers - 2:
                cur_downsample_layer = PatchMerging
            else:
